...
def load_demo_config(path: str) -> Tuple[str, int]:
    ...
    return phrase.strip(), speaker_id
...
class SpeakerRunner:
    ...
    def greet(self) -> None:
        if not self.enabled:
            return
        if self.dry_run:
            print(f"speaker: would say {self.phrase!r}")
            return

        with self._lock:
            if self._thread is not None and self._thread.is_alive():
                return
            self._thread = threading.Thread(target=self._speak, name="speaker_tts", daemon=True)
            self._thread.start()

    def _speak(self) -> None:
        try:
            ret = self.client.TtsMaker(self.phrase, self.speaker_id)
