...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    if args.execute_arm_sdk and not args.confirm_arm_ownership_reviewed:
        parser.error("publication requires --confirm-arm-ownership-reviewed")
    if args.execute_arm_sdk and args.safe_return_pose_rad is None:
        parser.error("publication requires 14 reviewed values in --safe-return-pose-rad")
...
class ArmSdkCommandSink:
    ...
    def __call__(self, positions: Sequence[float], velocities: Sequence[float], weight: float) -> None:
    ...
    try:
        ...
        if args.capture_post_action_pose:
            ...
            if args.execute_arm_sdk:
                ...
                event("arm_sdk_publisher_initialized", sink.configuration())
            else:
                sink = lambda *_: None
