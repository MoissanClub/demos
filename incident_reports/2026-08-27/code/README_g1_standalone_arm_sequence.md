...
near 250 Hz with bounded schedule lag, and successful high-level return.
...
rate-limited to 100 Hz to reduce JSON serialization contention. Inspect the generated JSONL file under
`telemetry/standalone_arm/` before progressing.

## Publication gate

`--execute-arm-sdk` exists for the later reviewed stage and adds a required
`--confirm-arm-ownership-reviewed` acknowledgement and fourteen reviewed
`--safe-return-pose-rad` values. The acknowledgement is not approval by itself.
Command publication remains suspended until the incident return-to-test
criteria and the command-only evidence are reviewed.
